...
function color(a){ return html.getAttribute('data-theme')==='dark'?`rgba(242,239,233,${a})`:`rgba(28,26,24,${a})`; }
function mk(){ return {x:Math.random()*canvas.width,y:Math.random()*canvas.height,r:Math.random()*.9+.3,vx:(Math.random()-.5)*.15,vy:(Math.random()-.5)*.15,a:Math.random()*.3+.05,p:Math.random()*Math.PI*2}; }
function initParticles(){
  if(!canvas) return;
  cancelAnimationFrame(raf); resize();
  pts = Array.from({length:Math.min(Math.floor(canvas.width*canvas.height/16000),70)},mk);
  loop();
}
function loop(){
  ctx.clearRect(0,0,canvas.width,canvas.height);
  pts.forEach(p=>{
    p.x+=p.vx; p.y+=p.vy; p.p+=.007;
  ...
  for(let i=0;i<pts.length;i++) for(let j=i+1;j<pts.length;j++){
    ...
    if(d<110){ ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle=color((1-d/110)*.05); ctx.lineWidth=.5; ctx.stroke(); }
  }
...
function getSlideWidth(){
  ...
  if(!s) return 0;
  return s.offsetWidth + parseInt(getComputedStyle(carousel).gap||'16');
}
...
function scrollToIndex(idx){
  ...
  currentIndex = idx;
  carousel.scrollTo({left: visibleSlides[idx].offsetLeft - (parseInt(getComputedStyle(carousel).paddingLeft)||0), behavior:'smooth'});
  updateDots();
}
...
carousel.addEventListener('mousedown',e=>{ isDragging=true; startX=e.clientX; carousel.style.cursor='grabbing'; });
carousel.addEventListener('mousemove',e=>{ if(!isDragging) return; carousel.scrollLeft -= (e.clientX - startX)*.5; });
carousel.addEventListener('mouseup',e=>{ isDragging=false; carousel.style.cursor=''; snapAfterDrag(); });
carousel.addEventListener('mouseleave',()=>{ isDragging=false; carousel.style.cursor=''; });
carousel.addEventListener('touchstart',e=>{ startX=e.touches[0].clientX; },{passive:true});
carousel.addEventListener('touchend',e=>{ const dx=e.changedTouches[0].clientX-startX; if(Math.abs(dx)>40){ dx<0?scrollToIndex(currentIndex+1):scrollToIndex(currentIndex-1); } },{passive:true});
...
const obs=new IntersectionObserver(entries=>{
  entries.forEach(e=>{
    ...
    const siblings=[...e.target.parentElement.children].filter(el=>el.classList.contains('reveal')&&!el.classList.contains('visible'));
...
new IntersectionObserver(entries=>{
  entries.forEach(e=>{ if(e.isIntersecting) nas.forEach(a=>a.classList.toggle('active-nav',a.getAttribute('href')==='#'+e.target.id)); });
},{rootMargin:'-50% 0px -50% 0px'}).observe.call(new IntersectionObserver(()=>{}),document.body)||
  secs.forEach(s=>new IntersectionObserver(entries=>{entries.forEach(e=>{if(e.isIntersecting)nas.forEach(a=>a.classList.toggle('active-nav',a.getAttribute('href')==='#'+e.target.id));});},{rootMargin:'-50% 0px -50% 0px'}).observe(s));
